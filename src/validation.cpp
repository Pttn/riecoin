                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Also, currently the rule against blocks more than 15 s in the future
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Speed up sync by checking the PoW of only some random blocks. This was included in Riecoin Core 0.16 and 0.20 (in a modified form as no block of less than 14 days would be skipped). Most would download the Bootstrap anyway so it reduces the purpose of this. Left commented as it is still useful for development purposes.
    bool checkPOW(fCheckPOW);
    /* if (checkPOW && ::ChainstateActive().IsInitialBlockDownload())
        checkPOW = (GetRand(16) == 0);*/
 ...
 */
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (params.NetworkIDString() == CBaseChainParams::REGTEST) // Fix some Tests
        maxFutureBlockTime = 7200;
    if (block.GetBlockTime() > nTime + maxFutureBlockTime)
        return state.Invalid(BlockValidationResult::BLOCK_TIME_FUTURE, "time-too-new", "block timestamp too far in the future");

    if (nHeight >= consensusParams.fork2Height) { // Stricter Timestamp Check starting from Fork 2 (note that the MAX_FUTURE_BLOCK_TIME above was also reduced to 15 s, and we do no longer have to check for the Fork)
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!ContextualCheckBlockHeader(block, state, chainparams, pindexPrev, GetTime()))
            return error("%s: Consensus::ContextualCheckBlockHeader: %s, %s", __func__, hash.ToString(), state.ToString());

        bool checkPOW(true);
        /*if (::ChainstateActive().IsInitialBlockDownload())
            checkPOW = (GetRand(16) == 0); */ // See Comment at Line 3305
...
bool ChainstateManager::ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, BlockValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex)
{
    ...
    if (NotifyHeaderTip()) {
        if (::ChainstateActive().IsInitialBlockDownload() && ppindex && *ppindex) {
            LogPrintf("Synchronizing blockheaders, height: %d (~%.2f%%)\n", (*ppindex)->nHeight, 100.0/((*ppindex)->nHeight+(GetTime() - (*ppindex)->GetBlockTime()) / Params().GetConsensus().nPowTargetSpacing) * (*ppindex)->nHeight);
        }
    }
    return true;
}
